...
export class BalancesService {
  ...
  private async getGroupExpenseData(groupId: string) {
  ...
  private async getGroupSettlementData(groupId: string) {
  ...
  private async getGroupBalances(groupId: string) {
  ...
  private calculateBalances(expenses: any[], settlements: any[]) {
    ...
    for (const expense of expenses) {
      ...
      balances.set(
        expense.paidById,
        (balances.get(expense.paidById) || 0) + amount,
      );
      ...
      for (const participant of expense.participants) {
        ...
        balances.set(
          participant.userId,
          (balances.get(participant.userId) || 0) - owed,
        );
      }
    }
    for (const settlement of settlements) {
      ...
      balances.set(
        settlement.paidById,
        (balances.get(settlement.paidById) || 0) + amount,
      );

      balances.set(
        settlement.receivedById,
        (balances.get(settlement.receivedById) || 0) - amount,
      );
    }
    return new Map(
      [...balances.entries()].map(([userId, balance]) => [
        userId,
        Math.abs(balance) < 0.01 ? 0 : Number(balance.toFixed(2)),
      ]),
    );
  }

  private simplifyDebts(balances: Map<string, number>) {
    const creditors: {
      userId: string;
      amount: number;
    }[] = [];
  ...
  async getBalances(userId: string, groupId: string) {
    ...
    return members
      .map((member) => ({
        ...
        balance: balances.get(member.user.id) || 0,
      }))
  ...
  async getGroupSummary(userId: string, groupId: string) {
    await this.validateGroupMembership(userId, groupId);
    const [expenseAggregate, expenseCount, memberCount] =
    ...
    return {
      totalExpenses: Number(expenseAggregate._sum.amount ?? 0),
  ...
  async getSimplifiedBalances(userId: string, groupId: string) {
    await this.validateGroupMembership(userId, groupId);
    const balances = await this.getGroupBalances(groupId);
